...
describe("Domain Exceptions", () => {
  describe("InsufficientTicketsException", () => {
    ...
    it("should work with different ticket types", () => {
      ...
      const generalException = new InsufficientTicketsException(
        "GENERAL",
        100,
        50,
      );
    ...
    it("should handle zero availability", () => {
      ...
      expect(exception.message).toBe(
        "Requested 1 VIP tickets but only 0 available",
      );
    });
    ...
    it("should handle large numbers", () => {
      ...
      expect(exception.message).toBe(
        "Requested 1000 GENERAL tickets but only 999 available",
      );
    });
  });
  ...
  describe("InvalidStateTransitionException", () => {
    it("should create exception with correct message format", () => {
      ...
      expect(exception.message).toBe(
        "Cannot confirm reservation in ACTIVE state",
      );
    ...
    it("should work with different state transitions", () => {
      ...
      transitions.forEach(({ from, action }) => {
        ...
        expect(exception.message).toBe(
          `Cannot ${action} reservation in ${from} state`,
        );
      });
    });
    ...
    it("should work with any action string", () => {
      ...
      expect(exception.message).toBe(
        "Cannot validate reservation in ACTIVE state",
      );
    });
  });
  ...
  describe("TicketTypeNotFoundException", () => {
    it("should create exception with correct message", () => {
      ...
      expect(exception.message).toBe(
        "Ticket type 'PREMIUM' not found in event configuration",
      );
    ...
    it("should work with different ticket types", () => {
      ...
      ticketTypes.forEach((type) => {
        ...
        expect(exception.message).toBe(
          `Ticket type '${type}' not found in event configuration`,
        );
      });
    });
    ...
    it("should handle special characters in ticket type", () => {
      ...
      expect(exception.message).toBe(
        "Ticket type 'VIP-PLUS' not found in event configuration",
      );
    });
  });
  ...
  describe("Exception Hierarchy", () => {
    ...
    it("exception names should match class names", () => {
      const testCases = [
        {
          exception: new InsufficientTicketsException("VIP", 1, 0),
          expectedName: "InsufficientTicketsException",
        },
        {
          exception: new InvalidEmailException("test"),
          expectedName: "InvalidEmailException",
        },
        {
          exception: new InvalidMoneyException("test"),
          expectedName: "InvalidMoneyException",
        },
        {
          exception: new InvalidQuantityException("test"),
          expectedName: "InvalidQuantityException",
        },
        {
          exception: new InvalidStateTransitionException("ACTIVE", "cancel"),
          expectedName: "InvalidStateTransitionException",
        },
        {
          exception: new TicketTypeNotFoundException("VIP"),
          expectedName: "TicketTypeNotFoundException",
        },
      ];
