...
describe('GetTicketStatsUseCase', () => {
  ...
  describe('execute with eventId', () => {
    it('should return ticket statistics for specific event', async () => {
      ...
      expect(mockTicketRepository.countTotalByEvent).toHaveBeenCalledWith(eventId);
      expect(mockTicketRepository.countSoldByEvent).toHaveBeenCalledWith(eventId);
      expect(mockTicketRepository.countUsedByEvent).toHaveBeenCalledWith(eventId);
      ...
      expect(mockTicketRepository.getSalesTrendForEvent).toHaveBeenCalledWith(eventId);
    });
    ...
    it('should execute all queries in parallel', async () => {
      ...
      expect(mockTicketRepository.getSalesTrendForEvent).toHaveBeenCalledTimes(1);
    });
  ...
  describe('execute without eventId', () => {
    it('should return all tickets statistics', async () => {
      ...
      const topSellingEvents = [
        { eventId: 'EVENT-001', eventName: 'Concert A', ticketsSold: 500, revenue: 25000 },
        { eventId: 'EVENT-002', eventName: 'Concert B', ticketsSold: 450, revenue: 22500 },
      ];

      mockTicketRepository.countSold.mockResolvedValue(2700);
      mockTicketRepository.getTotalRevenue.mockResolvedValue(191000);
      mockTicketRepository.getTicketsByStatus.mockResolvedValue(ticketsByStatus);
      ...
      mockTicketRepository.getTopSellingEvents.mockResolvedValue(topSellingEvents);
